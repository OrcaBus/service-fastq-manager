...
def get_q20_fraction(summary_dict: Dict[str, Union[float, int, str]]) -> float:
    ...
    :return:
    """
    if summary_dict.get("total_bases", 0):
        return 0
    return round(summary_dict.get("q20_bases", 0) / summary_dict.get("total_bases"), 2)
...
def get_gc_fraction(summary_dict: Dict[str, Union[float, int, str]]) -> float:
    ...
    if summary_dict.get("total_bases", 0) == 0:
        return 0

    return round(summary_dict.get("total_gc_bases", 0) / summary_dict.get("total_bases"), 2)
...
def main():
    ...
    output_dict = {
        ...
        "r1Q20Fraction": get_q20_fraction(data['summary']) if data.get('total_bases', 0) > 0 else 0,
        "r2Q20Fraction": (
            get_q20_fraction(data['summary_read2'])
            if data.get('total_bases', 0) > 0 else 0
